...
const getBaseOptions = cb => {
  ...
  if (process.env.COVERALLS_SERVICE_JOB_ID) {
    console.log(`resetting service_job_id ${options.service_job_id} TO ${process.env.COVERALLS_SERVICE_JOB_ID}`);
  ...
  const coveralls_yaml_conf = (() => {
    ...
    try {
      if (fs.statSync(yml).isFile()) {
        return yaml.load(fs.readFileSync(yml, 'utf8'));
      }
    } catch (_error) {
